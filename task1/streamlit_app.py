...
# Training the model with MC Dropout
def train_model_with_dropout(model, train_loader, epochs, criterion, optimizer):
    model.train()
...
def predict_with_dropout(model, x_val, n_samples):
    model.eval()
    with torch.no_grad():
...
def plot_contour(X_val, avg_prob, basis_function="No Transformation"):
    plt.figure()
    ...
    probs = predict_with_dropout(model, X_grid, mc_dropout_samples)
    ...
    plt.title(basis_function)
    fig = plt.gcf()
    st.pyplot(fig)

def preprocess_data(X, y, train_ratio):
...
st.title("MLP with MC Dropout - Basis Function Exploration")
...
enable_MC_dropout = st.sidebar.selectbox("MC Dropout", ("Disabled", "Enabled"))

num_datapoints = st.sidebar.slider("Number of Datapoints", min_value=100, max_value=5000, value=100)
...
if basis_function == "No Transformation":
    train_loader, X_train, X_val, y_train, y_val = preprocess_data(X, y, train_ratio)
elif basis_function == "Sine Basis":
    train_loader, X_train, X_val, y_train, y_val = preprocess_data(sine_basis(X), y, train_ratio)
elif basis_function == "Gaussian Basis":
    train_loader, X_train, X_val, y_train, y_val = preprocess_data(gaussian_basis(X), y, train_ratio)
elif basis_function == "Polynomial Basis":
    train_loader, X_train, X_val, y_train, y_val = preprocess_data(polynomial_basis(X), y, train_ratio)
else:
    st.write("Invalid Basis Function Selection.")
...
if enable_MC_dropout == "Enabled":
    mc_dropout_samples = st.sidebar.slider("MC Droput Samples", min_value=1, max_value=20, value=1)
else:
...
output_size=1
model = MLPWithDropout(input_size, hidden_layer_neurons, output_size, dropout_prob=dropout_prob)
criterion = nn.BCELoss()
optimizer = optim.SGD(model.parameters(), lr=learning_rate)
train_model_with_dropout(model, train_loader, epochs, criterion, optimizer)
avg_prob_no_transformation = predict_with_dropout(model, X_val, mc_dropout_samples)
plot_contour(X_val, avg_prob_no_transformation, basis_function)
